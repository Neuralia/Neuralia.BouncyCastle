...
namespace Neuralia.BouncyCastle.extra.pqc.crypto.Chacha {
		...
		: IStreamCipher
	{
		...
		private ByteArray	 keyStream = ByteArray.Create(StateSize * 4); // expanded state, 64 bytes
		...
		public Salsa20Engine2(int rounds)
		{
			...
			ICipherParameters	parameters)
		{
			...
			ParametersWithIV2 ivParams = parameters as ParametersWithIV2;
			if (ivParams == null)
            ...
            else if (keyParam is KeyParameter2)
            {
                SetKey(((KeyParameter2)keyParam).GetKey(), iv);
            }
            else
            {
        ...
        public void ProcessBytes(byte[] input, int inOff, int length, byte[] output, int outOff) {
	        ...
	        this.ProcessBytes(ByteArray.Wrap(input), inOff, length, ByteArray.Wrap(output), outOff);
        }
		...
		protected virtual void SetKey(ByteArray keyBytes, ByteArray ivBytes)
		{
            if (keyBytes != null)
            {
                ...
                // Key
                Pack.LE_To_UInt32(keyBytes.Bytes, keyBytes.Offset, engineState, 1, 4);
                Pack.LE_To_UInt32(keyBytes.Bytes, keyBytes.Offset+keyBytes.Length - 16, engineState, 11, 4);
            }

            // IV
            Pack.LE_To_UInt32(ivBytes.Bytes, ivBytes.Offset, engineState, 6, 2);
        }

        protected virtual void GenerateKeyStream(ByteArray output)
		{
